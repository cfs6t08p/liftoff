...
struct ign_call_table {
  ...
  int16_t (*get_encoder_count)(); // get encoder count (TODO: specify range)
  ...
  uint16_t (*ign_get_motor_current)();
  uint16_t (*ign_get_raw_battery_level)();
  uint16_t (*ign_get_touch_data)(uint8_t channel);
